...
import { setRoleDatabases, setRoleTables } from "../role_signals.js";
...
const ROLE_DATABASE_LIST_DETAILS_ROUTE_TEMPLATE = createRouteTemplate(
  ({ rolname }) => {
    const [
      readRoleDatabaseListDetailsOpened,
      storeRoleDatabaseListDetailsOpened,
      eraseRoleDatabaseListDetailsOpened,
    ] = valueInLocalStorage(`role_${rolname}_database_list_details_opened`, {
      type: "boolean",
    });

    return {
      match: () => readRoleDatabaseListDetailsOpened(),
      enter: () => {
        storeRoleDatabaseListDetailsOpened(true);
      },
      leave: () => {
        eraseRoleDatabaseListDetailsOpened();
      },
      load: async ({ signal }) => {
        ...
        if (!response.ok) {
          throw await errorFromResponse(
            response,
            `Failed to get databases for role ${rolname}`,
          );
        }
        const { data } = await response.json();
        setRoleDatabases(rolname, data);
        return data;
      },
      name: `role_${rolname}_database_list_details`,
    };
  },
);
export const getRoleDatabaseListDetailsRoute = (role) => {
  return ROLE_DATABASE_LIST_DETAILS_ROUTE_TEMPLATE.instantiate({
    rolname: role.rolname,
  });
};
