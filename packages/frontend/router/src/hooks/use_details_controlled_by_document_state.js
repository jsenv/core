...
import { useRouteIsMatching } from "../route/route_hooks.js";

export const useDetailsControlledByDocumentState = (
  routeUpdatingDocumentState,
  openByDefault = false,
) => {
  const routeIsMatching = useRouteIsMatching(routeUpdatingDocumentState);
  const open = routeIsMatching;
  ...
  useEffect(() => {
    ...
    if (openByDefault) {
      routeUpdatingDocumentState.enter();
    }
  }, []);
  ...
  return {
    ...
    onToggle: (toggleEvent) => {
      ...
      if (toggleEvent.newState === "open") {
        ...
        routeUpdatingDocumentState.leave();
      }
    },
  };
};
