 ...
 */
export const parseCSSBorder = (borderValue) => {
  ...
  if (normalizedValue === "0px solid transparent" || normalizedValue === "transparent") {
    return {
      width: 0,
      style: "solid",
      color: "transparent",
  ...
  for (const part of parts) {
    ...
    if (/^\d/.test(part) || /\d+(?:px|em|rem|ex|ch|vw|vh|vmin|vmax|cm|mm|in|pt|pc)$/.test(part)) {
