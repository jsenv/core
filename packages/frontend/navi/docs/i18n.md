...
replaced. The language is resolved per key: the preferred languages in order
(`lang`, or the whole `languagesSignal` list by default), then the
`fallbackLang` given at creation, and the first one translating the key wins. A
half-translated language therefore shows its own words where it has them and the
next language's everywhere else.

When none of them translates the key, it comes back as is, so a missing
translation degrades to readable text rather than blank space or a crash. With
opaque keys that text is the key's name, which is why an instance with opaque
keys wants a `fallbackLang` holding every key — `naviI18n` falls back to
English.
...
preferring German reads German wherever German exists — navi's time wording,
every date and number — and the next language it lists, or English, for the
rest: navi's buttons and messages, the app's own texts. One page, two
languages. With it, that browser lands on the app's best available language for
all of them.

It decides more than the wording: `<Time>` and the format helpers
(`formatNumber`, `formatTime`, …) read the same signal, so **a document that never calls it follows the browser,
dates and numbers included** — `2026-09-01` reads "09/01/2026" on an en-US
machine and "01/09/2026" on a French one, from the same code and the same data.
Nothing on screen says which reading is being applied, so this is worth one call
at startup even in an app with a single language, and worth remembering for a
second document in the same repo (an admin panel, a demo page), which starts
from zero.
