...
export const SlideContainer = ({
  ...
  const areaInUrl = urlParamName
    ? new URL(documentUrlSignal.value).searchParams.get(urlParamName)
    : null;
  ...
  const writeUrlParam = (area, cause, { replace = false } = {}) => {
    if (!urlParamName) {
      return;
    }
  ...
  const goBackToRefusedArea = (leftArea) => {
    ...
    writeUrlParam(leftArea, "code", { replace: true });
  };
  ...
  useLayoutEffect(() => {
    ...
    const areaOnScreen =
      containerRef.current?.getAttribute("data-slide-current");
    ...
    if (reached && reached !== areaOnScreen) {
      ...
      tellCurrentChange(reached, { cause: "url" }, areaOnScreen);
      return;
    }
