...
| `inspectResponse`            | `(request, { response, warn })`                                                 | look at the final response, after every injection, before it is sent (`response` is `{ status, statusText, headers, body }`); `warn` logs for that request |
