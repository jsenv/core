...
import { evalSource } from "../node-client/evalSource.js"
...
const execute = async ({
  ...
  const systemHref = `${compileServerOrigin}/${compileInto}/SYSTEM.js`
  const systemResponse = await fetchUsingHttp(`${compileServerOrigin}/${compileInto}/SYSTEM.js`)
  ...
  const { executeCompiledFile } = await global.System.import(
    `${compileServerOrigin}/${compileInto}/JSENV_NODE_CLIENT.js`,
  )
