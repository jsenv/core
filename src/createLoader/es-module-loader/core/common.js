...
export function fileUrlToPath(fileUrl) {
	...
	if (isWindows) return fileUrl.substr(8).replace(/\\/g, "/")
	else return fileUrl.substr(7)
}
...
function LoaderError__Check_error_message_for_loader_stack(childErr, newMessage) {
	...
	var err
	if (errArgs && childErr.fileName) err = new Error(message, childErr.fileName, childErr.lineNumber)
