...
import path, { resolve, extname, basename, dirname, relative as relative$1 } from "node:path";
import process$1, { env } from "node:process";
import { win32, posix, isAbsolute as isAbsolute$1, resolve as resolve$1 } from "path";
...
function requireNative () {
	...
	const { parse, parseAsync, xxhashBase64Url, xxhashBase36, xxhashBase16 } = requireWithFriendlyError(
		existsSync(path$1.join(import.meta.url.slice('file:///'.length).replace(/[\/\\][^\/\\]*$/, ''), localName)) ? localName : `@rollup/rollup-${packageBase}`
	);
...
var nativeExports = requireNative();

var parse = nativeExports.parse;
var parseAsync = nativeExports.parseAsync;
var xxhashBase16 = nativeExports.xxhashBase16;
var xxhashBase36 = nativeExports.xxhashBase36;
var xxhashBase64Url = nativeExports.xxhashBase64Url;
...
const nodeConverters = [
    ...
    function jsxAttribute(position, buffer) {
        ...
        return {
            ...
            name: convertNode$1(buffer[position + 2], buffer),
    ...
    function jsxElement(position, buffer) {
        ...
        return {
            ...
            openingElement: convertNode$1(buffer[position + 2], buffer),
            children: convertNodeList$1(buffer[position + 3], buffer),
            closingElement: closingElementPosition === 0 ? null : convertNode$1(closingElementPosition, buffer)
        };
    },
    ...
    function jsxFragment(position, buffer) {
        return {
            ...
            openingFragment: convertNode$1(buffer[position + 2], buffer),
            children: convertNodeList$1(buffer[position + 3], buffer),
            closingFragment: convertNode$1(buffer[position + 4], buffer)
        };
    },
    ...
    function jsxMemberExpression(position, buffer) {
        return {
            ...
            object: convertNode$1(buffer[position + 2], buffer),
    ...
    function jsxNamespacedName(position, buffer) {
        return {
            ...
            namespace: convertNode$1(buffer[position + 2], buffer),
            name: convertNode$1(buffer[position + 3], buffer)
        };
    },
...
function convertNodeList$1(position, buffer) {
    ...
    for (let index = 0; index < length; index++) {
        ...
        list[index] = nodePosition ? convertNode$1(nodePosition, buffer) : null;
    }
    return list;
}
...
const parseAst = (input, { allowReturnOutsideFunction = false, jsx = false } = {}) => convertProgram$1(getAstBuffer(parse(input, allowReturnOutsideFunction, jsx)));
...
for (const global of ['window', 'global', 'self', 'globalThis']) {
    knownGlobals[globalThis] = knownGlobals;
}
...
const rollup = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
	__proto__: null,
	VERSION: version,
	defineConfig,
	rollup: rollup$1,
...
export { createFilter, fseventsImporter, getAugmentedNamespace, getDefaultExportFromCjs, rollup, rollupInternal };
