...
var inspect = exports.inspect = function inspect(_ref) {
  ...
  return (0, _list.list)({ rootLocation: rootLocation, cacheFolderRelativeLocation: cacheFolderRelativeLocation }).then(function (folders) {
    return Promise.all(folders.map(function (folder) {
      return (0, _readFile.readFile)({ location: (0, _helpers.resolvePath)(cacheFolderLocation, folder, _cache.JSON_FILE) }).then(JSON.parse).then(function (cache) {
      ...
      return foldersStatus.map(function (status, index) {
        return { folder: folders[index], status: status };
      });
    });
  });
};
