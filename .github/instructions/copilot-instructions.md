...
- **JSDoc for exported components**: Use `@type {import("...").FunctionComponent<{ ... }>}` style JSDoc on exported components (see `packages/frontend/navi/src/control/list/list.jsx` for example). This enables VSCode prop autocomplete without needing separate `.d.ts` files.
