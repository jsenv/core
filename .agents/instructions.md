...
- **Never run the whole test suite on your own initiative**: it's long and expensive. Run a single test file, or at most a narrow subset (`npm run test -- ./tests/<some_directory>/`) targeted at what you changed. The full run (`npm run test` with no argument, `npm run test:packages`) only happens when the user explicitly asks for it. "Targeted at what you changed" means the tests that exercise the change's **consumers**, not the changed file's own directory: for a hub module (an action system, a store, a signal primitive) the file's neighbouring unit tests prove nothing about the routing, lists or forms built on it, and the honest subset is the integration tests of what depends on it — a hub file's top-of-file comment should name that net (see `navi/src/action/actions.js`), and when it doesn't, find the net before concluding "no regression".
